...
static u8 DPS_Config(void * Data)
{
	...
	Set_Cycle_Size(0);		//list清零
	/* Set DPS Config Flag */
	Set_Dps_Cfg_Flag();//DPS配置OK
...
static u8 Control_Command(void * Data)
{
	...
	switch (point->AgeStatus) {
		case AGING_DO:
			if(FIRST_SP == Get_FirstFlag())	//第一次启动完成
		...
		case AGING_CTE:
			...
			break;
		case AGING_CPL:
			System_Set_StopEven(AGE_OK);	//上位机发送老化完成
			break;
		case AGING_STOP:
...
static u8 Set_Pattern_Status(void * Data)
{
	...
	if(0x8 == setptr->Run_state)	//启动老化
	{
		shellprintln("    Start aging ...");
		SETCFGBIT(8);			    //开启fpga配置
		pat_r->RunSta = RUNNING;
		status = 0x0;

	}
	else if(0x0 == setptr->Run_state)   //停止
	{
...
static u8 Cycle_Info(void * Data)
{
	...
	Length = (temp->length) - HEAD_SIZE - COM_SIZE - LENGTH_SIZE - 4;//最后减4字节信息头

	if(cycle->cycle_crt_fir_frame !=  cycle->cycle_total_fir_frame)
	{
		if(cycle->cycle_crt_fir_frame == 1)
		{
			fpga_obj.list = (List *)LIST_ADDR;//复位list地址
			CycleSize = 0x00;
		}
	...
	else if(cycle->cycle_crt_fir_frame ==  cycle->cycle_total_fir_frame)
	{
		if((CycleSize + Length) >= CYCLE_DATA_SIZE)
		{
			...
			Length = CYCLE_DATA_SIZE-CycleSize; //剩余空间
			memcpy(desdaddr, point, Length);
		}
		else
		{
